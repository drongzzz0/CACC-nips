...
| --- | --- | ---: | --- |
| MMLU-Pro | SPP | 0.2663 | long Qwen3-8B generation still running; last checked 2026-06-08 04:25 CST. Four active parent/child route processes were visible on `ser`, with compute PIDs `795452`, `986649`, `995662`, and `1151365` on GPUs 0-3. The original non-checkpointed job is alive but has not written a candidate pool or final summary yet. Backup chunked-b4 run `repro20260607_mmlu_pro_spp_direct_qwen8b_full_reconstructed_chunked_b4_v1` reached `processed 1320/12032` at about `16.81s/ex`. Backup chunked-b8 run `repro20260607_mmlu_pro_spp_direct_qwen8b_full_reconstructed_chunked_b8_v1` reached `processed 2500/12032` at about `8.81s/ex`. Checkpointed b8 run `repro20260608_mmlu_pro_spp_direct_qwen8b_full_reconstructed_checkpointed_b8_v1` reached `processed 1728/12032`, with progress JSON partial first/oracle accuracies `0.235532 / 0.539931`; independent rescoring of the growing partial candidate JSONL with the project evaluator produced `0.236007 / 0.539527` over 1733 rows. No final summary exists yet. The dataset has 12032 examples, so this remains a long-running generation job rather than a failed result; current partial oracle evidence is close to the paper SPP verifier component (`0.5411`) but is not final O/V/F evidence. |
