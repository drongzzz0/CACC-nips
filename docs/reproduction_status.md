...
| --- | --- | ---: | --- |
| MMLU-Pro | SPP | 0.2663 | long Qwen3-8B generation still running; last checked 2026-06-08 04:11 CST. Four active parent/child route processes were visible on `ser`, with compute PIDs `795452`, `986649`, `995662`, and `1151365` on GPUs 0-3. The original non-checkpointed job is alive but has not written a candidate pool or final summary yet. Backup chunked-b4 run `repro20260607_mmlu_pro_spp_direct_qwen8b_full_reconstructed_chunked_b4_v1` reached `processed 1270/12032` at about `16.81s/ex`. Backup chunked-b8 run `repro20260607_mmlu_pro_spp_direct_qwen8b_full_reconstructed_chunked_b8_v1` reached `processed 2400/12032` at about `8.82s/ex`. Checkpointed b8 run `repro20260608_mmlu_pro_spp_direct_qwen8b_full_reconstructed_checkpointed_b8_v1` reached `processed 1630/12032`, with partial first/oracle accuracies `0.233742 / 0.534356`; it has a growing partial candidate JSONL and progress JSON but no final summary yet. The dataset has 12032 examples, so this remains a long-running generation job rather than a failed result. |
