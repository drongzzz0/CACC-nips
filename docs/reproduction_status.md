...
| --- | --- | ---: | --- |
| MMLU-Pro | SPP | 0.2663 | long Qwen3-8B generation still running; last checked 2026-06-08 04:33 CST. Four active parent/child route processes were visible on `ser`, with compute PIDs `795452`, `986649`, `995662`, and `1151365` on GPUs 0-3. The original non-checkpointed job is alive but has not written a candidate pool or final summary yet. Backup chunked-b4 run `repro20260607_mmlu_pro_spp_direct_qwen8b_full_reconstructed_chunked_b4_v1` reached `processed 1350/12032` at about `16.80s/ex`. Backup chunked-b8 run `repro20260607_mmlu_pro_spp_direct_qwen8b_full_reconstructed_chunked_b8_v1` reached `processed 2550/12032` at about `8.80s/ex`. Checkpointed b8 run `repro20260608_mmlu_pro_spp_direct_qwen8b_full_reconstructed_checkpointed_b8_v1` reached `processed 1782/12032`, with progress JSON and independent rescoring both giving partial first/oracle accuracies `0.234007 / 0.537037`. No final summary exists yet. The dataset has 12032 examples, so this remains a long-running generation job rather than a failed result. Current partial oracle evidence is close to the paper SPP verifier component (`0.5411`), but it only covers business and most law examples so far; math, physics, chemistry, engineering, and other remaining categories are still unobserved. Treat this as reassuring early risk evidence, not final O/V/F evidence. |
